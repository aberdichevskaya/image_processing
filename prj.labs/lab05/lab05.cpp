...
void borders_visualization(const std::vector<cv::Point2i>& points, 
	const cv::Mat& img, size_t idx) {
		cv::Mat res;
		img.copyTo(res);
		for (uint32_t i_point = 0; i_point < points.size(); i_point += 1) {
...
cv::Mat rotate_img(cv::Mat& img, const cv::Mat& scan, std::vector<cv::Point2i>& img_pts,
	const std::vector<cv::Point2i>& target_pts, size_t idx) {
	//std::cout << img_pts.size() << " " << target_pts.size();
...
void error_illustration(const cv::Mat1b& img, const cv::Mat1b& standard, const size_t num, const size_t idx) {
	cv::Mat3b dev_img;
...
cv::Mat create_etalon_binarization(const cv::Mat& scan) {
	cv::Mat1b etalon;
	cv::adaptiveThreshold(scan, etalon, 255, cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY, 19, 10);
	filtration(etalon);
	return etalon;
}
...
double process(cv::Mat& img, const cv::Mat1b& etalon, const size_t idx, const size_t num, 
	...
	error_illustration(img, etalon, num, idx);
	std::cout << "Results of the image number " + std::to_string(idx+1) + ": " <<  score0 << std::endl;
...
int main() {
	...
	std::vector<cv::Point2i> scan_special_pts = { {0, 0}, {scan_img.cols - 1, 0},{scan_img.cols - 1, scan_img.rows - 1},
													{0, scan_img.rows - 1}};


	std::vector<std::vector<cv::Point2i>> special_pts = {{{380,484},{2209,511},{2306,3215},{305,3232}},
									{{400,611}, {2009,609},{2031,2938} ,{379, 2918}}, //ok
									{{534,1966},{449,294}, {3175,61},{3088,2165}}, //ok
									{{612,1996},{694,488}, {3020,203},{3127,2168}}, //ok
									{{2839,450}, {2786,2169},{472,2060}, {469,454}}};

	cv::Mat rotated_img[num_img];
	cv::Mat etalon = create_etalon_binarization(scan_img);
	...
	std::cout << "First version of results:" << std::endl;
	for (ptrdiff_t i = 0; i < num_img; i = i + 1) {
		average_score1 += process(rotated_img[i], etalon, i, 1, 19, 10, 3);
	}
	std::cout << "Average score: " << average_score1/ num_img << std::endl;
	std::cout << std::endl;

	double average_score2 = 0.0;
	std::cout << "Second version of results:" << std::endl;
	for (ptrdiff_t i = 0; i < num_img; i = i + 1) {
